...
class VolunteerRepository @Inject constructor(
    ...
    fun getVolunteerProfile(uid: String): Flow<Resource<Volunteer>> {
        if (secureStorage.isDemoMode()) {
            return flow {
                emit(Resource.Loading)
                emit(Resource.Success(getMockVolunteer()))
            }
        }
    ...
    private fun getMockVolunteer(): Volunteer {
        return Volunteer(
            uid = "demo_user_id",
            name = "Demo Volunteer",
            email = "volunteer@demo.com",
            skills = listOf("First Aid", "Driving"),
            xp = 1500,
            completedTasks = 12
        )
    }
